...
class MySQL {
	...
	function ExecuteSQL($query, $keys=true){
	...
	// Adds a record to the database or update on ducplicate key
	function InsertDupKey($table, $fields, $values, $key){
		
		$set=array();
		foreach($fields as $linekey=>$field) {
			if($field!=$key) {
				$set[]= "`".$field."`='".$values[$linekey]."'";
			}
		}
		...
		$query = "INSERT INTO `".$table."` (".$fields.") VALUES (".$values.") ON DUPLICATE KEY UPDATE ".$set;

		return $this->ExecuteSQL($query);
	}
	
	// Adds a record to the database : custom review
	function InsertCustom($table, $fields, $values){
	...
	function ArrayResults($keys=true){
